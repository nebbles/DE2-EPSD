...
BEAT_THRESHOLD = 1.69		# threshold for c to indicate a beat
...
while True:				# Main program loop
	...
	pot_val = int(100 * pot.read() / 4096)
	oled.draw_text(0, 40, 'POT Value:{:5d}%'.format(pot_val))
	oled.display()
	# if (pot_val >= dead_up):
	# 	pot_val = (pot_val - 49) * 2
	# elif (pot_val <= dead_down):
	# 	pot_val = (49 - pot_val) * 2
	# else:
	# 	pass

	# modify the threshold value for sensitivity
	# display the threshold on oled screen
