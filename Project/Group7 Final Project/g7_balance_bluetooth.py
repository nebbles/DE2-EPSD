'''
-------------------------------------------------------
Name: Group 7 Self-Balancing, Beat Detection & Bluetooth
Creator: Benedict Greenberg
Date: 23 March 2017
Revision: 1.0
-------------------------------------------------------
Same as g7_balance_beat.py with added Bluetooth control
to influence self-balance behaviour.
-------------------------------------------------------
'''
import pyb, time
...
oled.draw_text(0,10, 'Balance, Beat, Btooth')
oled.draw_text(0,20, 'Press USR button')
oled.display()


print('g7_balance_bluetooth.py')
	...
	while True:
		while uart.any() != 10:
			...
			if dt > 5000:		# sampling time is 5 msec or 50Hz
				...
				if u > 0:
					motor.A_forward( (abs(u)+motor_offset) * A_weight )
					motor.B_forward( (abs(u)+motor_offset) * B_weight )
				elif u < 0:
					motor.A_back( (abs(u)+motor_offset) * A_weight )
					motor.B_back( (abs(u)+motor_offset) * B_weight )
				else:
					motor.A_stop()
					motor.B_stop()
		...
		if command[2] == ord('5'): # UP pressed to increase tilt
			pidc.target_inc(0.2)

		elif command[2] == ord('6'): # UP pressed to decrease tilt
			pidc.target_dec(0.2)

		elif command[2] == ord('7'): # LEFT was pressed
			if (A_weight + 0.1) < 1.7: # as long as weighting will be no more than 1.7
				A_weight += 0.1 # increase weighting
				B_weight -= 0.1

		elif command[2] == ord('8'):
			if (A_weight - 0.1) > 0.3: # as long as weighting will be no less than 0.3
				A_weight -= 0.1 # decrease weighting
				B_weight += 0.1

		elif command[2] == ord('1'): # '1' was pressed so reset tilt
			pidc.target_reset()

		elif command[2] == ord('2'): # '2' was pressed so reset turning
			A_weight = 1.0
			B_weight = 1.0
